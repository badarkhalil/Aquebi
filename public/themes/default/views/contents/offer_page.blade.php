<div class="container">
  <div class="offered-product-widget space30">
    <span class="offered-product-widget-img">
      <img class="lazy" src="/images/loading.webp" data-src="{{ get_storage_file_url(optional($product->featuredImage)->path, 'medium') }}" alt="{!! $product->name !!}" title="{!! $product->name !!}" />
    </span>
      ...
      @foreach ($product->inventories->sortBy(function ($item) {
        ...
        <tr class="sc-product-item">
          ...
          <td class="seller-info">
            <div class="space10">
              <img src="/images/loading.webp" data-src="{{ get_storage_file_url(optional($offer->shop->image)->path, 'thumbnail') }}" class="lazy seller-info-logo img-sm" alt="{{ trans('theme.logo') }}">
          ...
          <td>
            <a class="btn btn-default flat btn-block btn-sm itemQuickView" href="javascript:void(0);" data-link="{{ route('quickView.product', $offer->slug) }}" rel="nofollow noindex">
