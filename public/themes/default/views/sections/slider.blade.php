  ...
  <div id="ei-slider" class="ei-slider">
    <ul class="ei-slider-large">
      @foreach ($sliders as $slider)
        <li>
          <a href="{{ $slider['link'] }}">
            <img class="lazy" src="/images/loading.webp" data-src="{{ get_storage_file_url($slider['feature_image']['path'] ?? null, 'full') }}" alt="{{ $slider['title'] ?? 'Slider Image ' . $loop->count }}">
          </a>
    ...
    <ul class="ei-slider-thumbs">
      ...
      @foreach ($sliders as $slider)
        <li>
          ...
          <img class="lazy" src="/images/loading.webp" data-src="{{ get_storage_file_url($slider['images'][0]['path'] ?? ($slider['feature_image']['path'] ?? null), 'slider_thumb') }}" alt="thumbnail {{ $loop->count }}" />
        </li>
      @endforeach
    </ul>
  </div>
