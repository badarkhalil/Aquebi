  ...
  <div class="modal-content">
    <div class="modal-body" style="padding: 0px;">
      <button type="button" class="close" data-dismiss="modal" aria-hidden="true"
              style="position: absolute; top: 5px; right: 10px; z-index: 9;">×
      </button>

      <div class="col-md-3 nopadding" style="margin-top: 10px;">
        @if($cart->customer->image)
          <img src="{{ get_storage_file_url(optional($cart->customer->image)->path, 'small') }}" class="thumbnail"
               width="100%" alt="{{ trans('app.avatar') }}">
        @else
          <img src="{{ get_gravatar_url($cart->customer->email, 'small') }}" class="thumbnail" width="100%"
              ...
              @if(count($cart->inventories) > 0)
                @foreach($cart->inventories as $item )
                  <tr>
                    <td>
                      @if($item->image)
                        <img src="{{ get_storage_file_url($item->image->path, 'tiny') }}" class="img-circle img-md"
                             alt="{{ trans('app.image') }}">
                      @elseif($item->product->featuredImage)
                        <img src="{{ get_storage_file_url($item->product->featuredImage->path, 'tiny') }}"
                             class="img-circle img-md" alt="{{ trans('app.image') }}">
                      @else
                        <img src="{{ get_storage_file_url(optional($item->product->image)->path, 'tiny') }}"
                             class="img-circle img-md" alt="{{ trans('app.image') }}">
                      @endif
                    </td>
          ...
          <div class="tab-pane" id="invoice_tab">
            <table class="table no-border">
              <tr>
                ...
                <td style="width: 75%;"><span class="lead"> {{ get_formated_currency($cart->grand_total, 2) }}</span>
                </td>
              </tr>
              <tr>
