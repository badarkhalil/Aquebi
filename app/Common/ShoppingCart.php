...
trait ShoppingCart
{
    ...
    public function addToCart(Request $request, $slug)
    {
        ...
        if ($customer_id) {
            $old_cart = Cart::where('shop_id', $item->shop_id)->where('customer_id', $customer_id);

            if (!empty(Session::get('session_carts'))) {
                $old_cart = $old_cart->orWhere(function ($q) {
                    $q->whereNull('customer_id')->whereIn('id', Session::get('session_carts'));
                });
            }
            ...
            $old_cart = Cart::where('shop_id', $item->shop_id)
                ...
                ->whereIn('id', Session::get('session_carts'))->first();
        }
